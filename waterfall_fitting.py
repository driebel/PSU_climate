...
parser.add_argument("infile", help="Enter the file you wish to process",default='gui_select')
parser.add_argument("-boot_num",help="Enter the number of bootstrap loops",
                    type=int, default = 100)
args = parser.parse_args()

if args.infile == 'gui_select':
    waterfall_file = easygui.fileopenbox(title='Select the Waterfall file to be Processed')
else:
    waterfall_file = args.infile
    
boot_num = args.boot_num
waterfall_dir = os.path.dirname(waterfall_file)+'/'
base = os.path.basename(waterfall_file)[:-4]
home_dir = os.path.split(os.path.dirname(waterfall_file))[0]+'/'
var_dir = home_dir + 'variability_data/'
fit_dir = home_dir + 'quadratic_fits/'
model_type = 'era'
scenario=''

print 'Beginning Waterfall Bootstrap'                         
...
if os.path.exists(waterfall_file):
    ...
    outdir = waterfall_dir+'output/'
    print 'There better be files in ', outdir
    ...
    #create idl script to run batch job of surface plot routine
    batch_file = 'surface_batch.pro'
    with open(batch_file,'w') as f:
        f.write("waterfall_model_plot, '"+waterfall_file+"','"+model_file+"'\n")
        f.write('exit')
...
print 'Bootstrap portion took '+str(time.time() - start)      
